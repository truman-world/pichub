...
<h1 align="center">PicHub - 企业级图像资产管理平台</h1>

<p align="center">
  <img alt="PHP版本" src="https://img.shields.io/badge/PHP-%3E%3D8.2-777BB4?logo=php">
  <img alt="框架" src="https://img.shields.io/badge/Laravel-v11.x-FF2D20?logo=laravel">
...
- **Composer**: PHP 包管理器
- **必需的PHP扩展**:
  - `pdo_pgsql` (用于PostgreSQL) 或 `pdo_mysql` (用于MySQL)
  - `bcmath`
  - `ctype`
  - `fileinfo`
  - `mbstring`
...
2.  **安装依赖**
    ...
    ```
3.  **配置环境变量**
    复制 `.env.example` 为 `.env`，并填入您的数据库连接信息和应用密钥。
    ```bash
    cp .env.example .env
...
5.  **启动开发环境**
    ...
    php artisan serve
    ```
    首次启动，请访问 `http://localhost:8000`，系统将自动引导您进入安装页面。
